...
def get_suite_context(request, path):
    ...
    subsuites = [
        {"link_name": d, "pretty_name": pretty_variable(d)}
    ...
    return {
        ...
        'html_stubs': [
            {'path': file[1].path(file[0]),
                'file': os.path.basename(file[0]).rstrip(".html")}
...
def walk_finders(path):
    ...
    # By adding this second, files in the project base override app specific
    # files
